  ...
  function App() {
    ...
    const fetchWeather = async (lat, lon) => {
      ...
      try {
        ...
        if (!data.hourly || !data.hourly.time || data.hourly.time.length === 0) {
            displayError("No hourly weatherData data available for this location.");
            return;
        }
        ...
        // Map weatherData code to a descriptive string
        ...
        // Set the processed weatherData data
        setWeatherData({
    ...
    function setBackgroundcolor(weatherType){
      ...
      let backgroundColor = "";
          if (
            ...
            weatherType === "Freezing Drizzle: Light" ||
            weatherType === "Freezing Drizzle: Dense intensity" ||
            weatherType === "Rain: Slight" ||
            weatherType === "Rain: Moderate" ||
            weatherType === "Rain: Heavy intensity" ||
            weatherType === "Freezing Rain: Light" ||
            weatherType === "Freezing Rain: Heavy intensity" ||
    ...
    return (
      ...
      <div className={`${weatherData ? setBackgroundcolor(weatherData.weatherType) : "bg-slate-500"}  transition-colors duration-500 min-h-screen p-3 flex flex-col items-center`} >
       {!weatherData && <p className="text-white">Loading weather data...</p>}
        ...
        {weatherData && (
          <div className="mt-1">
            <WeatherCard location={locations.find(l => l.name === selectedLocation)} weatherData={weatherData} />
          </div>
        )}
      </div>
    );
